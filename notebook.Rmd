...
We can now plot the network with a customize legend indicating the previously specified attriubute-colour/shape/size combinations.
...
We can calculate indegree and outdegree by counting the incoming and outgoing ties or simply by using the sna function *degree*.
...
We can also plot the previous graph by colouring Sara (in orange) and Francesco (in red) nodes:
...
# Prepare darker color vector for Sara and Francesco nodes
...
Looking at the results we can say that there is a high correlation between the grade and the indegree (how much a  person is bullied). It is possible to see that this result is statistically significant given its low p-value. Moreover, we can derive that a person with high grades tends to be bullied more, as we saw in Sara's example and in the scatterplot above.
...
message("Indegree centralization: ", indegree_centralization_network)
message("Maximum indegree centralization: ", max_indegree_centralization)
message("Normalized indegree centralization: ", round(normalized_indegree_centralization_network, 2))

message("Outdegree centralization: ", outdegree_centralization_network)
message("Maximum outdegree centralization: ", max_outdegree_centralization)
message("Normalized outdegree centralization: ", round(normalized_outdegree_centralization_network, 2))
...
As can be seen, the normalized values of outdegree and indegree centralization are not very high, which means that in both cases there is no very central node. This means that no one is bullied by the majority of the network or bullies most of the classmates.
...
```
Sna's `grecip()` uses the following formula to obtain the index reciprocity value:
$$
